...
CREATE TABLE IF NOT EXISTS `mydb`.`Врач` (
  ...
  `ФИО` VARCHAR(45) NOT NULL,
  `Дата рождения` DATE NOT NULL,
  `ID-спецмализации` INT NOT NULL,
  `Должность_idДолжность` INT NOT NULL,
  PRIMARY KEY (`idВрач`),
  INDEX `fk_Врач_Карта врача1_idx` (`ID-спецмализации` ASC) VISIBLE,
  INDEX `fk_Врач_Должность1_idx` (`Должность_idДолжность` ASC) VISIBLE,
  CONSTRAINT `fk_Врач_Карта врача1`
    FOREIGN KEY (`ID-спецмализации`)
    REFERENCES `mydb`.`Специализация` (`ID-специализации`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
